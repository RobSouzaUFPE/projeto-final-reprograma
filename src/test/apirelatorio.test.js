...
*/
 ...
 test("Rota Get - Listar relatório por nome da criança", (done) => {
    request(app)
        .get("/clinica/relatorio/nome?nome_crianca=I")
        .expect(200)
        .expect((res) => {
...
test("Rota Delete - /clinica/relatorio/delete/id=", (done) =>{
    request(app)
        .delete(`/clinica/relatorio/delete/id=${elementId}`)
        .expect(200)
        .expect((res) =>{
            console.log(res.body);
            expect(res.body.message).toBe(`Relatório com id ${elementId} foi excluído com sucesso.`)
            })
            .end((err,res) =>{
                if(err) return done(err)
                return done()
